...
namespace Transition
{
    public class RandomStaticCollection : CollectionBase
    {
        ...
        public void RandomStatic(ushort X, ushort Y, ushort Z, Collection[,] StaticMap)
        {
            IEnumerator enumerator = this.InnerList.GetEnumerator();
            try
            {
                ...
                {
                    RandomStatic randomStatic = (RandomStatic)enumerator.Current;
                    StaticCell item = new StaticCell(randomStatic.TileID, checked((byte)(unchecked(X + randomStatic.X) % 8)), checked((byte)(unchecked(Y + randomStatic.Y) % 8)), (ushort)(Z + randomStatic.Z));
                    StaticMap[(int)((ushort)(X + randomStatic.X >> 3)), (int)((ushort)(Y + randomStatic.Y >> 3))].Add(item, null, null, null);
                }
            }
            finally
            {
